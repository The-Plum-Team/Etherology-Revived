...
particle IDs and their server-side payload/codec/wire contracts. `SharedMaterialItems` now owns
14 behavior-free items with exact packaged models, textures, English names, and bounded native
server registry/NBT evidence. `SharedMetalBlocks` and `SharedMetalBlockItems` now own exact
`azel_block`, `ethril_block`, and `ebony_block` blocks and corresponding block items, with bounded
native server registry/property/tag/NBT/placement evidence. Channel cases, client particle
...
`SharedFoodItems` owns the plain `forest_lantern_crumb` with exact food behavior,
model/texture/name resources, and bounded native player-consumption evidence.
`SharedForestLanternBlocks` and `SharedForestLanternBlockItems` now own the paired block/item and
...
- `validateForgeParticleRegistryMilestone` is an accepted predecessor gate. It requires the
...
- `validateForgeMaterialItemRegistryStaticMilestone` is expected to pass. It proves
  `SharedMaterialItems` is the sole Common deferred owner of the exact 14 behavior-free item IDs,
  checks exact fields and registration order, rejects eager supplier resolution and direct
  registration, requires one loader attachment, excludes the former eager fields, and verifies
  byte-exact models, textures, and English names across the Common and loader artifacts.
- `validateForgeMaterialItemRegistryServerEvidenceArchiveIntegrity` is expected to pass. It runs
  the pinned v11 runner/verifier safety suite and validates the immutable five-file schema-7,
  163-assertion archive without consulting ignored live runtime state.
- `validateForgeMaterialItemRegistryMilestone` is an accepted predecessor gate. It requires
  the static/bytecode/resource/artifact proof and frozen v11 native reload evidence before the
  remaining authoritative registry gate may run.
- `validateForgeMetalBlockRegistryStaticMilestone` is expected to pass. It proves
  `SharedMetalBlocks` and `SharedMetalBlockItems` are the sole Common deferred owners of the exact
  three metal blocks and corresponding block items, checks exact settings and mappings, and
  validates models, textures, English names, loot tables, recipes, and selected tags across the
  Common and loader artifacts. In the packaged `mineable/pickaxe` and `needs_iron_tool` files,
...
- `validateForgeFoodItemRegistryStaticMilestone` is expected to pass. It proves
  ...
  texture, English/Russian names, and loader attachments. The later Forest Lantern static gate
  validates the now-packaged three recipes and three matching advancements.
- `validateForgeFoodItemRegistryServerEvidenceArchiveIntegrity` is expected to pass. It runs the
  pinned v14 runner/verifier safety suite and validates the immutable five-file schema-9,
...
- `validateForgeFoodItemRegistryMilestone` is an accepted predecessor gate. It requires
...
- `validateForgeForestLanternStaticMilestone` is expected to pass. It proves the shared
  Forest Lantern block/item declaration, twenty-state and mechanic contract, exact data and
  render resources, both loader aliases and initialization paths, and Forge cutout registration.
- `validateForgeForestLanternServerEvidenceArchiveIntegrity` validates the immutable five-file
...
persistence, multiplayer, or release readiness. The later consumed Fabric v24
Forest Lantern profile passed all 68 assertions and seven native 1920x1080
captures; any next run requires v25 or newer. The Fabric `v23` metal-block and `v22` Phase 0
...
## Accepted bounded SharedMaterialItems registry and server milestone

`SharedMaterialItems` is the sole Common deferred owner of these 14 behavior-free item IDs:
...
Fabric attaches the owner directly before its retained item initializer, while Forge attaches the
same owner through `EtherologyBootstrap` before registry events. The former eager fields are
absent from Fabric's retained `EItems` and `DecoBlockItems` classes. Every declaration supplies a
vanilla `Item`; `enriched_attrahite` has maximum stack count 16 and the other 13 have maximum
stack count 64.

`validateForgeMaterialItemRegistryStaticMilestone` checks the exact owner, supplier fields,
registration order, loader attachment, no direct `Registry.register`, no eager supplier `get`, and
absence of the legacy fields. It applies those checks across the Common JAR, both transformed
Common JARs, the Fabric development and remapped production JARs, and the Forge shadow JAR. The
three loader application artifacts must also package byte-exact models and textures and exact
English names for all 14 entries.

`validateForgeMaterialItemRegistryMilestone` combines that static proof with the immutable
[`material-item-registry-server-v11`](../evidence/forge-1.20.1/material-item-registry-server-v11)
archive. The fresh repository-owned `etherology-e2e-forge-server-1.20.1-v11` profile ran the
headless `material-item-registry` scenario through
`:forge:1.20.1:runRegistryFoundationServerProbe` on Java 17 and Forge 47.4.9. Report schema 7
passed all 163 ordered assertions. After server-data load, at server start, and after a real
`reload`, all 14 entries resolved to `net.minecraft.item.Item` with the exact maximum counts.
Maximum-count stacks serialized the exact ID/count and `Count` plus `id` NBT keys and
round-tripped to the same item/count. The cumulative run also re-proved the v10 particle, v7
enchantment, game-event/tag, loot-condition, and Ether-source contracts. The world saved,
`stop(false)` completed, the launcher exited zero, and the copied log contains no fatal or
forbidden client-startup marker.
...
This native proof is limited to registry properties and in-process `ItemStack` NBT
round-trip/reload stability. It creates no screenshots, does not give an item to a player with
`/give`, and does not launch a second JVM or restart the server. Forge fuel registration,
creative-tab placement, recipes, client rendering/gameplay, and all consumer mechanics remain
unproved.

## Accepted bounded SharedMetalBlocks registry and server milestone

`SharedMetalBlocks` and `SharedMetalBlockItems` are the sole Common deferred owners of
`etherology:azel_block`, `etherology:ethril_block`, and `etherology:ebony_block` as three vanilla
`Block` instances and their exact mapped `BlockItem`s. Azel copies iron-block settings with
`MapColor.LAPIS_BLUE`, ethril copies gold-block settings, and ebony copies diamond-block settings
with `MapColor.ORANGE`. All three retain metal sound, tool requirements, pickaxe/iron-tool tag
membership, and maximum item count 64; only ethril and ebony are beacon bases.

The static milestone checks sole ownership, lazy registration and attachment order, exact
block-item map enrollment, removal of the former eager `DecoBlocks` fields, and packaged
models/textures/names, self-drop tables, compression/decompression recipes, and selected tags. In
the `mineable/pickaxe` and `needs_iron_tool` files packaged by this bounded Forge slice,
still-unported IDs are optional while the three accepted metal-block IDs remain required.
`needs_stone_tool` is unchanged and outside this Forge resource slice. This prevents
partial-catalog tag-load failures without registering placeholder blocks.

The fresh repository-owned `etherology-e2e-forge-server-1.20.1-v13` profile ran the headless
`metal-block-registry` scenario on Java 17 and Forge 47.4.9. Report schema 8 passed all 188 ordered
assertions. It proved exact registry classes and mappings, block properties, selected tags, and
stack NBT after data load and at server start, directly placed the three exact block IDs at bounded
world positions, and rechecked all of that state after a real `reload`. The cumulative run also
re-proved v11 and every earlier registry/reload contract. The world saved, normal `stop(false)`
completed, and the launcher exited zero. The immutable archive is
[`metal-block-registry-server-v13`](../evidence/forge-1.20.1/metal-block-registry-server-v13), with
...
The v12 diagnostic profile was consumed by the required-tag-reference failure and was never
accepted or archived. The successful v13 proof remains bounded: it does not establish restart
persistence, player `/give` or player placement, mining/drop behavior, beacon activation, recipe
execution, creative-tab interaction, client rendering, the full authoritative registry, or a
complete port. Its immutable archive proves capture-time Loom-userdev observations and payload
...
At the v14 food-item checkpoint, the three original recipes and three matching advancements
referenced the then-unported `forest_lantern`, so those six resources were deliberately deferred
rather than packaged with an invalid dependency or changed ingredient. The later accepted Forest
Lantern slice packages and verifies all six.
...
`e2bba0e01d27a7f9f4511d00b2d3fa3a12c8d9fa4b5aaa74cca09ca536d599db`. The pinned v14 Python runner
...
This proof is headless and has no client visual evidence. At this checkpoint it did not establish
a second JVM or restart persistence, multiplayer, the deferred recipe behavior,
creative-tab interaction, client rendering/gameplay, the full authoritative registry, or release
readiness.

## Accepted bounded Forest Lantern milestone

`SharedForestLanternBlocks` and `SharedForestLanternBlockItems` are the sole Common deferred
owners of `etherology:forest_lantern`. The loader aliases point to those suppliers, and the Common
block implementation owns the exact twenty age/facing states, shapes, growth, bonemeal, support,
jump, mining, and loot behavior. The shared data/resources include the three crumb recipes and
advancements, age-gated loot, tags, models, textures, blockstate, names, and Forge cutout setup.

The fresh `etherology-e2e-forge-server-1.20.1-v16` Java 17 Forge 47.4.9 dedicated server passed
all 266 schema-10 assertions and stopped normally. Its five-file archive is
[`forest-lantern-server-v16`](../evidence/forge-1.20.1/forest-lantern-server-v16); the archive
manifest SHA-256 is
...
`45a628ddbe67dd90c0d713fa0788caebb17bff071689e2e05b45dbbaed4c8732` and
`2082b006646371e9339467bb59daa80d7d0c62d1102864ba56fa9b668c00dde9`.
Its immutable archive is
[`forest-lantern-v13`](../evidence/forge-1.20.1/forest-lantern-v13). The Fabric v24 companion
passed all 68 assertions and the same seven-capture persistence progression. All three accepted
profiles are consumed; any later Fabric client run requires v25 or newer and any later Forge
client run requires v14 or newer.

`validateForgeForestLanternMilestone` combines the static/artifact proof with both Forge archives;
the Fabric companion has its separate accepted milestone. These bounded proofs do not establish
...
remaining item, block-entity, and screen-handler catalogs must converge into one active declaration owner per
