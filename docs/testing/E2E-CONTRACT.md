...
| `attrahite-block-registry` | Exact raw Attrahite, Attrahite Bricks, slab, and stairs block/item mappings; runtime block classes, default states, complete state counts, non-negative network IDs, pickaxe/stone-tool/slab/stairs tags, and seventeen packaged render resources; before server fixture setup, 20 consecutive client ticks must prove that chunks `[-1,-1]`, `[-1,0]`, `[0,-1]`, and `[0,0]` are loaded, the client chunk-update queue is empty, the lighting provider has no pending updates, all four arena light columns are enabled, and all six pre-setup SKY samples equal 15; four real BlockItem placements on deterministic pedestals; exact four-table loot behavior including seeded plain, Silk Touch, and Fortune I–III raw-block drops; nine loaded recipes exercised through their real recipe implementations and nine exact recipe advancements; initial and post-reopen client/server fixture mirrors plus the latest client fixture snapshot for each phase, with native 1920×1080 captures after six paired local SKY/BLOCK samples report exact values `15/14`, `15/14`, `15/10`, `15/10`, `15/10`, and `15/8`, the server snapshot remains exact for 20 consecutive server END ticks, the client samples equal that snapshot for 20 consecutive client ticks, global post-setup pending-light flags remain diagnostic only, and 120 consecutive exact render-ready frames complete; forced save, full disconnect/reopen, exact fixture and loaded-data persistence, and normal shutdown. The harness does not invoke client-local relighting, direct client light-engine draining, light-packet construction, or packet resends. This bounded visual contract does not prove natural ore generation, player mining timing, creative-tab interaction, multiplayer, or broader registry completeness. |
| `slitherite-block-registry` | Exact seventeen-block Slitherite family and mapped block items, including runtime classes, default properties, 1,262 unique non-negative state network IDs, eleven exact block/item tags, 79 packaged visual resources, self-drop loot for all blocks, double-slab drops, 29 owned recipes and advancements, and five recorded related recipes; seventeen real `BlockItem` placements on deterministic supports; native button pulse/reset and pressure-plate entity filtering/reset; exact server and client fixture mirrors, deterministic pre-setup and capture-time lighting, and initial/reopened unedited 1920×1080 framebuffers after 120 consecutive exact render-ready frames; forced save, full disconnect/reopen, exact fixture and loaded-data persistence, and normal shutdown. The five related recipes are evidence of the original dependency graph, not ownership by this bounded block-family slice. This contract does not prove natural generation, player mining timing, recipe crafting by a player, creative-tab interaction, multiplayer, or broader registry completeness. |
...
| `forest-lantern` | `forge-1.20.1` | Accepted cumulative dedicated-server schema-10 proof in a fresh repository-owned Loom-userdev profile: exact shared block/item identity and mapping, mature north default, all twenty age×facing states with unique non-negative server network IDs and exact outline shapes, upstream-derived `notSolid` properties including the native 1.20.1 `opaque=true` state flag, hoe tag and deliberately empty peach-log tag; exact age-gated loot, four recipes and four recipe advancements loaded initially and after a real `reload`, including real recipe matching/crafting; real supported mature `BlockItem` placements in all four facings followed by support removal; a real grounded `ServerPlayerEntity` with shears proves mining speed `15.0` and the expected effective breaking delta across every one of the twenty states; separately seeded real player jumps prove the 40% callback's retain, single-callback guard, mature break and exact one-item drop paths using fresh players before and after reload; registry, states, tags, loaded data, and mechanics remain exact through reload; world save, normal stop, exit code zero, and no fatal, client-startup, or forbidden-class marker. This headless scenario requires no screenshots. Ages 0–3 cannot naturally attach while the peach-log/Golden-Forest graph is deferred, so natural growth and bonemeal spreading remain outside this bounded proof; it also does not prove a second JVM, multiplayer, client visuals, creative-tab interaction, the full authoritative registry, or release readiness. |

The accepted cumulative predecessor is `food-item-registry` report schema 9
...
The consumed v15 run completed its native save, reload, stop, and report
publication. It passed 262 assertions and failed four assertions that all
descended from one incorrect prepared oracle: the harness expected
`opaque=false`, while the upstream-derived 1.20.1 block state correctly
reported `opaque=true`. No v15 archive is accepted and that identity must never
be relaunched.

The current accepted cumulative record is `forest-lantern` report schema 10
with 266 of 266 passing assertions in
`docs/evidence/forge-1.20.1/forest-lantern-server-v16`. Its one-shot profile
manifest is 1,184 bytes with SHA-256
