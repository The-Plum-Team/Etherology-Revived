#!/usr/bin/env python3
"""Strict verifier for the immutable original published-0.1.7 Slitherite v10 lane."""
...
PROFILE_ID = "etherology-original-fabric-1.21.1-published-0.1.7-v10"
PROFILE_RELATIVE_PATH = (
    "scripts/baseline/original-fabric-1.21.1-published-0.1.7-v10.json"
)
HARNESS_VERSION = "1.3.5"
...
MAXIMUM_REOPEN_MATERIAL_CHANGED_PIXEL_RATIO = 0.15
MAXIMUM_REOPEN_MEAN_MAX_CHANNEL_DELTA = 16.0
...
EXPECTED_PRE_SETUP_SKY_LIGHT_LEVELS = [15] * 18
EXPECTED_ARENA_CHUNKS = [
    "-2,-2",
    "-2,-1",
    "-2,0",
    "-1,-2",
...
EXPECTED_PRE_SETUP_LIGHTING_DESCRIPTION = (
    "20 consecutive ticks: twelve chunks loaded; chunk update queue empty; "
    "lighting provider idle; twelve enabled columns; "
    f"sky={EXPECTED_PRE_SETUP_SKY_LIGHT_LEVELS}"
)
EXPECTED_CAMERA = (
...
class SlitheriteEvidenceError(RuntimeError):
    """Reports a fail-closed Slitherite v10 evidence-contract violation."""


def _fail(message: str, error_type: type[Exception]) -> None:
    raise error_type(f"Original Slitherite v10 evidence: {message}")
...
def _validate_pre_setup_lighting(
    ...
    evidence = _require_exact_fields(
        ...
        "pre-setup lighting evidence",
        error_type,
    )
    stable_client_ticks = evidence["stable_client_ticks"]
    enabled_columns = evidence["enabled_columns"]
    sky_levels = evidence["sky"]
    if (
        type(stable_client_ticks) is not int
        or stable_client_ticks != REQUIRED_PRE_SETUP_LIGHT_READY_CLIENT_TICKS
        or evidence["arena_chunks"] != EXPECTED_ARENA_CHUNKS
        or evidence["arena_chunks_loaded"] is not True
        or evidence["chunk_updaters_empty"] is not True
        or evidence["pending"] is not False
        or not isinstance(enabled_columns, list)
        or len(enabled_columns) != len(EXPECTED_ARENA_CHUNKS)
        or any(value is not True for value in enabled_columns)
        or not isinstance(sky_levels, list)
        or len(sky_levels) != len(EXPECTED_PRE_SETUP_SKY_LIGHT_LEVELS)
        or any(type(value) is not int for value in sky_levels)
        or sky_levels != EXPECTED_PRE_SETUP_SKY_LIGHT_LEVELS
    ):
        _fail("pre-setup client light payload proof is not exact", error_type)
...
def visual_drift_statistics(
    initial: PngLike, reopened: PngLike, error_type: type[Exception]
) -> tuple[float, float]:
    """Returns bounded material-change and mean per-pixel delta metrics."""
    ...
    for offset in range(0, len(initial.pixels), 3):
        ...
        maximum_channel_delta_total += maximum_channel_delta
        material_changes += maximum_channel_delta > MATERIAL_PIXEL_DELTA_THRESHOLD
    pixel_count = initial.width * initial.height
    material_ratio = material_changes / pixel_count
    mean_maximum_channel_delta = maximum_channel_delta_total / pixel_count
    if material_ratio > MAXIMUM_REOPEN_MATERIAL_CHANGED_PIXEL_RATIO:
        _fail(
            "reopened screenshot has too many materially changed pixels "
            f"({material_ratio:.6f})",
            error_type,
        )
    if mean_maximum_channel_delta > MAXIMUM_REOPEN_MEAN_MAX_CHANNEL_DELTA:
        _fail(
            "reopened screenshot has excessive mean maximum-channel delta "
            f"({mean_maximum_channel_delta:.6f})",
            error_type,
        )
    return material_ratio, mean_maximum_channel_delta
...
def _assertion_map(
    ...
    if not isinstance(assertions, list) or len(assertions) != len(EXPECTED_ASSERTION_NAMES):
        _fail("assertion count is not exactly 185", error_type)
...
def validate_evidence(
    *,
    ...
    """Validates one completed immutable v10 capture without trusting the harness."""
    ...
    if (
        ...
        _fail("report did not pass the exact v10 lifecycle", error_type)
    ...
    _validate_pre_setup_lighting(slitherite["pre_setup_lighting"], error_type)
    if (
    ...
    material_ratio, mean_maximum_channel_delta = visual_drift_statistics(
    ...
    return SlitheriteEvidenceSummary(
        ...
        reopen_mean_max_channel_delta=mean_maximum_channel_delta,
    )
