#!/usr/bin/env python3
"""Strict verifier for the immutable original published-0.1.7 Slitherite v5 lane."""
...
PROFILE_RELATIVE_PATH = (
    "scripts/baseline/original-fabric-1.21.1-published-0.1.7-v5.json"
)
...
class SlitheriteEvidenceError(RuntimeError):
    """Reports a fail-closed Slitherite v5 evidence-contract violation."""


def _fail(message: str, error_type: type[Exception]) -> None:
    raise error_type(f"Original Slitherite v5 evidence: {message}")
...
def validate_evidence(
    *,
    ...
    """Validates one completed immutable v5 capture without trusting the harness."""
    ...
    if (
        ...
        _fail("report did not pass the exact v5 lifecycle", error_type)
