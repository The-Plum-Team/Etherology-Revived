...
MODULE_PATH = BASELINE_DIRECTORY / "original_slitherite_evidence_v10.py"
SPECIFICATION = importlib.util.spec_from_file_location(
    "etherology_original_slitherite_evidence_v10_tested", MODULE_PATH
)
...
LOW_AMPLITUDE_DRIFT_IMAGE = FakeImage(
    1920, 1080, b"\xd0\xd0\xd0" * (1920 * 1080)
)
MEAN_DRIFT_IMAGE = FakeImage(1920, 1080, b"\xd9\xd9\xd9" * (1920 * 1080))
MATERIAL_CHANGE_PIXEL_COUNT = (1920 * 1080 * 16) // 100
MATERIAL_DRIFT_IMAGE = FakeImage(
    1920,
    1080,
    b"\x96\x96\x96" * MATERIAL_CHANGE_PIXEL_COUNT
    + b"\xc8\xc8\xc8" * (1920 * 1080 - MATERIAL_CHANGE_PIXEL_COUNT),
)
...
def contract_fixture(root: Path) -> tuple[Path, Path, dict[str, object], list[dict[str, object]]]:
    ...
    slitherite: dict[str, object] = {
        ...
        "pre_setup_lighting": {
            ...
            "arena_chunks": list(verifier.EXPECTED_ARENA_CHUNKS),
            "arena_chunks_loaded": True,
            "chunk_updaters_empty": True,
            "pending": False,
            "enabled_columns": [True] * 12,
            "sky": [15] * 18,
        },
    ...
    assertions["client_arena_chunks_loaded_before_setup"] = assertion(
        "client_arena_chunks_loaded_before_setup", "true", "true"
    )
    assertions["client_arena_light_payloads_applied_before_setup"] = assertion(
        "client_arena_light_payloads_applied_before_setup",
        verifier.EXPECTED_PRE_SETUP_LIGHTING_DESCRIPTION,
        verifier.EXPECTED_PRE_SETUP_LIGHTING_DESCRIPTION,
    )
...
class SlitheriteEvidenceV10Tests(unittest.TestCase):
    def test_visual_drift_bounds_are_exact(self) -> None:
        self.assertEqual(verifier.MATERIAL_PIXEL_DELTA_THRESHOLD, 24)
        self.assertEqual(
            verifier.MAXIMUM_REOPEN_MATERIAL_CHANGED_PIXEL_RATIO,
            0.15,
        )
        self.assertEqual(verifier.MAXIMUM_REOPEN_MEAN_MAX_CHANNEL_DELTA, 16.0)
    ...
    def test_complete_contract_passes_with_185_assertions_and_two_captures(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            scenario, world, report, artifacts = contract_fixture(
                Path(temporary_directory)
            )
            summary = self.validate(scenario, world, report, artifacts)
            self.assertEqual(summary.assertion_count, 185)
            self.assertEqual(summary.screenshot_count, 2)
            self.assertEqual(summary.reopen_material_changed_pixel_ratio, 0.0)
            self.assertEqual(summary.reopen_mean_max_channel_delta, 0.0)

    def test_pre_setup_light_payload_proof_fails_closed_for_every_gate(self) -> None:
        mutations = {
            "stable ticks": lambda value: value.__setitem__(
                "stable_client_ticks", 19
            ),
            "arena inventory": lambda value: value["arena_chunks"].reverse(),
            "arena chunks": lambda value: value.__setitem__(
                "arena_chunks_loaded", False
            ),
            "chunk queue": lambda value: value.__setitem__(
                "chunk_updaters_empty", False
            ),
            "lighting pending": lambda value: value.__setitem__("pending", True),
            "enabled column": lambda value: value["enabled_columns"].__setitem__(
                0, False
            ),
            "enabled column type": lambda value: value[
                "enabled_columns"
            ].__setitem__(0, 1),
            "enabled column inventory": lambda value: value[
                "enabled_columns"
            ].pop(),
            "sky sample": lambda value: value["sky"].__setitem__(0, 0),
            "sky sample type": lambda value: value["sky"].__setitem__(0, 15.0),
            "sky inventory": lambda value: value["sky"].pop(),
        }
        for description, mutate in mutations.items():
            with self.subTest(description=description):
                with tempfile.TemporaryDirectory() as temporary_directory:
                    ...
                    mutate(report["slitherite"]["pre_setup_lighting"])
                    with self.assertRaises(verifier.SlitheriteEvidenceError):
                        self.validate(scenario, world, report, artifacts)

    def test_pre_setup_named_assertion_is_mandatory_and_exact(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            scenario, world, report, artifacts = contract_fixture(
                Path(temporary_directory)
            )
            for value in report["assertions"]:
                if (
                    ...
                    value["actual"] = "stableClientTicks=19"
                    break
            with self.assertRaises(verifier.SlitheriteEvidenceError):
                self.validate(scenario, world, report, artifacts)
    ...
    def test_reopened_low_amplitude_render_drift_is_bounded(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            scenario, world, report, artifacts = contract_fixture(
                Path(temporary_directory)
            )
            summary = self.validate(
                ...
                images={
                    verifier.INITIAL_SCREENSHOT_FILE: BRIGHT_IMAGE,
                    verifier.REOPENED_SCREENSHOT_FILE: LOW_AMPLITUDE_DRIFT_IMAGE,
                },
            )
            self.assertEqual(summary.reopen_material_changed_pixel_ratio, 0.0)
            self.assertEqual(summary.reopen_mean_max_channel_delta, 8.0)

    def test_reopened_mean_render_drift_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            scenario, world, report, artifacts = contract_fixture(
                Path(temporary_directory)
            )
            with self.assertRaises(verifier.SlitheriteEvidenceError):
                self.validate(
                    ...
                    images={
                        verifier.INITIAL_SCREENSHOT_FILE: BRIGHT_IMAGE,
                        verifier.REOPENED_SCREENSHOT_FILE: MEAN_DRIFT_IMAGE,
                    },
                )

    def test_reopened_material_pixel_drift_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            scenario, world, report, artifacts = contract_fixture(
                Path(temporary_directory)
            )
            with self.assertRaises(verifier.SlitheriteEvidenceError):
                self.validate(
                    ...
                    images={
                        verifier.INITIAL_SCREENSHOT_FILE: BRIGHT_IMAGE,
                        verifier.REOPENED_SCREENSHOT_FILE: MATERIAL_DRIFT_IMAGE,
                    },
                )

    def test_reopened_dimension_drift_fails_closed(self) -> None:
        with self.assertRaises(verifier.SlitheriteEvidenceError):
            verifier.visual_drift_statistics(
                FakeImage(1, 1, b"\xc8\xc8\xc8"),
                FakeImage(2, 1, b"\xc8\xc8\xc8" * 2),
                verifier.SlitheriteEvidenceError,
            )
