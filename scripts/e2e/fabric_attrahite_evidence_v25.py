...
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile-v25.json"
PROFILE_SIZE = 7051
PROFILE_SHA256 = "cc28abb3a530fec7e4fa2e67f8825db371b07ad4774071a0d630a19edc702cc0"
HARNESS_SIZE = 286969
HARNESS_SHA256 = "98c7afa7652ed736f0e600bc68327a3644444dd339cfff8c2b4fe59ff60071e8"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v25"
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v25 verifier."""
...
def validate_active_profile(configuration: client.ResolvedConfiguration) -> None:
    """Requires the immutable v25 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise client.E2EError("The active Fabric profile differs from its v25 snapshot")
    ...
    if (
        ...
        raise client.E2EError("The Fabric Attrahite v25 profile contract changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v25 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise client.E2EError("Fabric Attrahite archive directory does not identify v25")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v25 to be the sole versioned Fabric Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise client.E2EError("The exact sole Fabric Attrahite v25 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v25 capture and copied payload."""
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact v25 profile")
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact owned v25 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v25 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v25 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v25 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v25 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v25 manifest used only while sealing"
    )
