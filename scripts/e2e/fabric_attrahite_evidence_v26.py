...
PROFILE_ID = "etherology-e2e-fabric-1.20.1-v26"
ACTIVE_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile.json"
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile-v26.json"
PROFILE_SIZE = 7051
PROFILE_SHA256 = "4091f9627f79b4ba816aa7a5fa90cf6731ea938884bb9aee9be642c3eb896862"
HARNESS_SIZE = 292255
HARNESS_SHA256 = "6ba6379b35e00d7a1b27f0c328fa652bcda9af4cc79a763cca609a17e604df4b"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v26"
...
EXPECTED_LIGHTING_DESCRIPTION = (
    "stableClientTicks=20;pending=false;"
    "sky=[0,121,-8=15,-3,123,1=15,-1,123,1=15,1,123,1=15,3,123,1=15];"
    "block=[0,121,-8=14,0,121,0=14]"
)
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v26 verifier."""
...
def validate_active_profile(configuration: client.ResolvedConfiguration) -> None:
    """Requires the immutable v26 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise client.E2EError("The active Fabric profile differs from its v26 snapshot")
    ...
    if (
        ...
        raise client.E2EError("The Fabric Attrahite v26 profile contract changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v26 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise client.E2EError("Fabric Attrahite archive directory does not identify v26")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v26 to be the sole versioned Fabric Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise client.E2EError("The exact sole Fabric Attrahite v26 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v26 capture and copied payload."""
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact v26 profile")
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact owned v26 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v26 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v26 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v26 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v26 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v26 manifest used only while sealing"
    )
