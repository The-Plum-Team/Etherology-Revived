...
PROFILE_ID = "etherology-e2e-fabric-1.20.1-v27"
ACTIVE_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile.json"
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile-v27.json"
PROFILE_SIZE = 7051
PROFILE_SHA256 = "b2ea801d26457b7b3ed807d04997cf76e8eaf2c10ca536231d29858a5ce323da"
HARNESS_SIZE = 296303
HARNESS_SHA256 = "710eb1846e50ace19aabe53b7494f100758a5db23fff9ff81e0722ff148e589c"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v27"
...
EXPECTED_BLOCK_LIGHT = dict(
    zip(LIGHT_SAMPLE_POSITIONS, (14, 14, 6, 4, 10, 8), strict=True)
)
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v27 verifier."""
...
def validate_active_profile(configuration: client.ResolvedConfiguration) -> None:
    """Requires the immutable v27 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise client.E2EError("The active Fabric profile differs from its v27 snapshot")
    ...
    if (
        ...
        raise client.E2EError("The Fabric Attrahite v27 profile contract changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v27 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise client.E2EError("Fabric Attrahite archive directory does not identify v27")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v27 to be the sole versioned Fabric Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise client.E2EError("The exact sole Fabric Attrahite v27 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v27 capture and copied payload."""
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact v27 profile")
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact owned v27 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v27 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v27 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v27 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v27 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v27 manifest used only while sealing"
    )
