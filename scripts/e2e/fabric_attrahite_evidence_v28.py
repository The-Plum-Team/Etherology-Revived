...
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile-v28.json"
PROFILE_SIZE = 7051
PROFILE_SHA256 = "34e187dae9cff37754238ce41b4dc707265d1d083df0a4f133b84b6780fc98e1"
HARNESS_SIZE = 296299
HARNESS_SHA256 = "0abc2b77b0090259510f536b0cd8f3f547eab8f2c00b71c7353b28d6947c73ee"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v28"
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v28 verifier."""
...
def validate_active_profile(configuration: client.ResolvedConfiguration) -> None:
    """Requires the immutable v28 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise client.E2EError("The active Fabric profile differs from its v28 snapshot")
    ...
    if (
        ...
        raise client.E2EError("The Fabric Attrahite v28 profile contract changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v28 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise client.E2EError("Fabric Attrahite archive directory does not identify v28")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v28 to be the sole versioned Fabric Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise client.E2EError("The exact sole Fabric Attrahite v28 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v28 capture and copied payload."""
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact v28 profile")
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact owned v28 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v28 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v28 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v28 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v28 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v28 manifest used only while sealing"
    )
