...
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/fabric-1.20.1-profile-v30.json"
PROFILE_SIZE = 7051
PROFILE_SHA256 = "f2131cb9190b17b42035604d26988a1aa8091cbc94541cb29c0e9d018bcf8000"
HARNESS_SIZE = 300673
HARNESS_SHA256 = "1c978b594d0f6d92355b1d588993cc979e47f4fb39548213c7ac17ed813d267a"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v30"
...
EXPECTED_PRE_SETUP_LIGHTING_DESCRIPTION = (
    ...
    "lighting provider idle; four enabled columns; "
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v30 verifier."""
...
def validate_active_profile(configuration: client.ResolvedConfiguration) -> None:
    """Requires the immutable v30 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise client.E2EError("The active Fabric profile differs from its v30 snapshot")
    ...
    if (
        ...
        raise client.E2EError("The Fabric Attrahite v30 profile contract changed")
...
def validate_pre_setup_lighting(pre_setup: object) -> None:
    if (
        ...
        raise client.E2EError("Fabric Attrahite pre-setup lighting fields changed")
    ...
    if (
        ...
        or not exact_light_map(
            pre_setup.get("sky"), EXPECTED_PRE_SETUP_SKY_LIGHT
        )
    ):
        raise client.E2EError("Fabric Attrahite pre-setup lighting changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v30 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise client.E2EError("Fabric Attrahite archive directory does not identify v30")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v30 to be the sole versioned Fabric Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise client.E2EError("The exact sole Fabric Attrahite v30 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v30 capture and copied payload."""
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact v30 profile")
    ...
    if (
        ...
        raise client.E2EError("Fabric Attrahite sealing requires the exact owned v30 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v30 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v30 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v30 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v30 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v30 manifest used only while sealing"
    )
