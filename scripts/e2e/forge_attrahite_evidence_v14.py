...
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/forge-1.20.1-profile-v14.json"
PROFILE_SIZE = 3702
PROFILE_SHA256 = "d880c523c6987836cfad5dfe9d640b1d4ee807664f3fc335ae5b31b6fbfe1e44"
HARNESS_SIZE = 226499
HARNESS_SHA256 = "652fd6ce40df7131ddd679b2cde5ae77f278e93d8631bf5ed74b9d3a0d523c10"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v14"
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v14 verifier."""
...
def validate_active_profile(configuration: forge_client.ResolvedConfiguration) -> None:
    """Requires the immutable v14 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise forge_client.E2EError("The active Forge profile differs from its v14 snapshot")
    ...
    if (
        ...
        raise forge_client.E2EError("The Forge Attrahite v14 profile contract changed")
...
def validate_screenshot_files(
    ...
    for screenshot in screenshots:
        ...
        images.append(image)
    ratio = structural_change_ratio(images[0], images[1])
    if ratio > MAXIMUM_REOPEN_STRUCTURAL_CHANGE_RATIO:
        raise forge_client.E2EError(
...
def validate_live_evidence(
    ...
    """Validates the exact live v14 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise forge_client.E2EError("Forge Attrahite archive directory does not identify v14")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v14 to be the sole versioned Forge Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise forge_client.E2EError("The exact sole Forge Attrahite v14 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v14 capture and copied payload."""
    ...
    if (
        ...
        raise forge_client.E2EError("Forge Attrahite sealing requires the exact v14 profile")
    ...
    if (
        ...
        raise forge_client.E2EError("Forge Attrahite sealing requires the exact owned v14 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v14 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v14 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v14 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v14 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v14 manifest used only while sealing"
    )
