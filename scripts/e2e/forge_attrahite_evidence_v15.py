...
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/forge-1.20.1-profile-v15.json"
PROFILE_SIZE = 3702
PROFILE_SHA256 = "7744609bfdc40ca69e86fb4e2d6bb4e2755d9072097acde54b7d5dd9c0537e71"
HARNESS_SIZE = 239889
HARNESS_SHA256 = "00de9d56d423edcb265ea61a35c1bbf5e2e966bdf448c15de01c363ecd778f75"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v15"
...
LIGHTING_SAMPLE_DESCRIPTION = (
    "[0,121,-8=15/14,-3,123,1=15/6,-1,123,1=15/4,"
    "1,123,1=15/10,3,123,1=15/8]"
)
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v15 verifier."""
...
def validate_active_profile(configuration: forge_client.ResolvedConfiguration) -> None:
    """Requires the immutable v15 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise forge_client.E2EError("The active Forge profile differs from its v15 snapshot")
    ...
    if (
        ...
        raise forge_client.E2EError("The Forge Attrahite v15 profile contract changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v15 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise forge_client.E2EError("Forge Attrahite archive directory does not identify v15")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v15 to be the sole versioned Forge Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise forge_client.E2EError("The exact sole Forge Attrahite v15 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v15 capture and copied payload."""
    ...
    if (
        ...
        raise forge_client.E2EError("Forge Attrahite sealing requires the exact v15 profile")
    ...
    if (
        ...
        raise forge_client.E2EError("Forge Attrahite sealing requires the exact owned v15 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v15 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v15 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v15 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v15 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v15 manifest used only while sealing"
    )
