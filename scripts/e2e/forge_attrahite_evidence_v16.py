...
SNAPSHOT_PROFILE_RELATIVE_PATH = "scripts/e2e/forge-1.20.1-profile-v16.json"
PROFILE_SIZE = 3702
PROFILE_SHA256 = "05162c31a5effae1efc3b1191a4dadb7f0ba5333fa993508bb1ee3ffecde7535"
HARNESS_SIZE = 239915
HARNESS_SHA256 = "ffb21364959114d247dfe04cd90c47ba0e7c24572ec0245d02b3a196536dea15"
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-v16"
...
class AttrahiteEvidenceSummary:
    """Reports the evidence inventory accepted by the v16 verifier."""
...
def validate_active_profile(configuration: forge_client.ResolvedConfiguration) -> None:
    """Requires the immutable v16 profile and exact runtime contract."""
    ...
    if active_profile.read_bytes() != snapshot_profile.read_bytes():
        raise forge_client.E2EError("The active Forge profile differs from its v16 snapshot")
    ...
    if (
        ...
        raise forge_client.E2EError("The Forge Attrahite v16 profile contract changed")
...
def validate_live_evidence(
    ...
    """Validates the exact live v16 capture without launching or changing it."""
...
def validate_archive_inventory(archive_root: Path, include_manifest: bool = True) -> None:
    ...
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise forge_client.E2EError("Forge Attrahite archive directory does not identify v16")
    files: set[str] = set()
...
def validate_no_competing_archives(archive_root: Path) -> None:
    """Requires v16 to be the sole versioned Forge Attrahite archive."""
    ...
    if candidates != [archive_root] or not archive_root.is_dir() or archive_root.is_symlink():
        raise forge_client.E2EError("The exact sole Forge Attrahite v16 archive is required")
...
def build_archive_manifest(
    ...
    """Builds a seal only from the exact live v16 capture and copied payload."""
    ...
    if (
        ...
        raise forge_client.E2EError("Forge Attrahite sealing requires the exact v16 profile")
    ...
    if (
        ...
        raise forge_client.E2EError("Forge Attrahite sealing requires the exact owned v16 runtime")
...
def validate_archived_evidence(archive_root: Path) -> AttrahiteEvidenceSummary:
    """Validates a self-contained v16 archive without consulting live state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned v16 capture runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v16 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v16 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime", type=Path, help="owned runtime used only while sealing"
    )
    parser.add_argument(
        "--profile-manifest", type=Path, help="v16 manifest used only while sealing"
    )
