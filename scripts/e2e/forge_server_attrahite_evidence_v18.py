...
PROFILE_MANIFEST_RELATIVE_PATH = Path(contract_v18.PROFILE_MANIFEST_RELATIVE_PATH)
...
ARCHIVE_DIRECTORY_NAME = "attrahite-block-registry-server-v18"
...
EXPECTED_ASSERTIONS = tuple(
    zip(
        contract_v18.EXPECTED_ASSERTION_NAMES,
        contract_v18.EXPECTED_ASSERTION_VALUES,
        strict=True,
    )
)
PROBE_LOG_PHASES = (
    *contract_v18.PROBE_LOG_PHASES,
    "loom_userdev_exit_scheduled",
)
TERMINATION_LOG_TOKEN = contract_v18.SERVER_LOG_TOKENS[-1]
...
def load_profile_manifest(path: Path = PROFILE_MANIFEST_PATH) -> ProfileRecord:
    ...
    if (
        record.size != contract_v18.PROFILE_MANIFEST_SIZE
        or record.sha256 != contract_v18.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The Forge server profile bytes differ from the immutable v18 contract"
        )
    return record
...
def validate_report(report: dict[str, object]) -> None:
    """Validates the report through the immutable profile-v18 contract."""
    try:
        ...
        raise EvidenceError(
            f"The Forge server report violates the v18 contract: {exception}"
        ) from exception
...
def validate_archive_root_identity(archive_root: Path) -> None:
    """Requires the immutable Attrahite block registry server-v18 identity."""
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise EvidenceError(
            "The Forge server archive directory does not identify "
            "Attrahite block registry profile v18"
        )


def validate_no_competing_attrahite_archives(repository_root: Path) -> None:
    """Rejects older or newer Attrahite block registry evidence beside v18."""
...
def validate_archive_manifest_shape(
    ...
    if (
        profile_record.size != contract_v18.PROFILE_MANIFEST_SIZE
        or profile_record.sha256 != contract_v18.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The archived Forge server profile differs from the immutable v18 contract"
        )
