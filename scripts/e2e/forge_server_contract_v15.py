"""Freeze the Forge 1.20.1 Forest Lantern report contract for profile v15."""
...
PROFILE_ID = "etherology-e2e-forge-server-1.20.1-v15"
...
FOREST_LANTERN_PROPERTIES = (
    ...
    "|grass_sound=true|tool_required=false|luminance=8|opaque=false"
...
class V15ContractError(RuntimeError):
    """Reports an exact profile-v15 report contract violation."""
...
def _validate_assertions(assertions: object) -> None:
    if not isinstance(assertions, list) or len(assertions) != len(
        ...
        raise V15ContractError("The server probe assertion inventory changed")
...
def _validate_forest_lantern(value: object) -> None:
    if not isinstance(value, dict) or set(value) != set(FOREST_LANTERN):
        raise V15ContractError("The server probe Forest Lantern result changed")
...
def validate_probe_report(
    ...
    """Validates the exact profile-v15 report and cumulative v14 projection."""
    expected_fields = {
        "schema",
        "profile_id",
        "scenario",
    ...
    if set(report) != expected_fields:
        raise V15ContractError("The server probe report field inventory changed")
    ...
    if not isinstance(reload, dict):
        raise V15ContractError("The server probe reload result changed")
    for field in (
        "forest_lantern_registry_stable",
        "forest_lantern_states_stable",
        "forest_lantern_tags_stable",
    ...
    try:
        ...
        raise V15ContractError(str(exception)) from exception
