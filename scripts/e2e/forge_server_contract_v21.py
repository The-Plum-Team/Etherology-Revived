"""Freeze the Forge 1.20.1 Slitherite server report contract for profile v21."""
...
PROFILE_ID = "etherology-e2e-forge-server-1.20.1-v21"
...
def _load_slitherite_contract() -> ModuleType:
    ...
    specification = importlib.util.spec_from_file_location(
        "etherology_slitherite_client_evidence_contract_v1_server_v21",
        path,
    )
...
class V21ContractError(RuntimeError):
    """Reports an exact profile-v21 report contract violation."""


def _validate_slitherite_blocks(value: object) -> None:
    if not isinstance(value, dict):
        raise V21ContractError("The server probe Slitherite result changed")
    entries = value.get("entries")
    if not isinstance(entries, dict) or tuple(entries) != SLITHERITE_BLOCK_IDS:
        raise V21ContractError("The server probe Slitherite entry inventory changed")

    all_raw_ids: list[int] = []
    for identifier in SLITHERITE_BLOCK_IDS:
        entry = entries.get(identifier)
        if not isinstance(entry, dict):
            raise V21ContractError(
    ...
    if (
        ...
        raise V21ContractError("The server probe Slitherite aggregate raw IDs changed")
    ...
    if type(elapsed) is not int or elapsed < 20:
        raise V21ContractError("The server probe Slitherite button timing changed")
...
def _validate_assertions(assertions: object) -> None:
    if not isinstance(assertions, list) or len(assertions) != len(
        ...
        raise V21ContractError("The server probe assertion inventory changed")
...
def validate_probe_report(
    ...
    if set(report) != expected_fields:
        raise V21ContractError("The server probe report field inventory changed")
    ...
    if not isinstance(reload, dict):
        raise V21ContractError("The server probe reload result changed")
    for field in _SLITHERITE_RELOAD_FIELDS:
    ...
    try:
        ...
        raise V21ContractError(str(exception)) from exception
