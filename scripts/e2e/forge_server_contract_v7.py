"""Freeze the Forge 1.20.1 enchantment-registry report contract for profile v7."""
...
PROFILE_ID = "etherology-e2e-forge-server-1.20.1-v7"
SCENARIO_ID = "enchantment-registry"
...
PROFILE_SNAPSHOT_RELATIVE_PATH = (
    "scripts/e2e/forge-server-1.20.1-profile-v7.json"
)
PROFILE_MANIFEST_SIZE = 1194
...
class V7ContractError(RuntimeError):
    """Reports an exact profile-v7 report contract violation."""
...
def validate_probe_report(
    ...
    """Validates the exact profile-v7 report and its profile mod alignment."""
    expected_fields = {
        "schema",
        "profile_id",
        "scenario",
    ...
    if set(report) != expected_fields:
        raise V7ContractError("The server probe report field inventory changed")
    expected_scalars = {
        "schema": 5,
    ...
    if (
        ...
        raise V7ContractError("The full loaded mod id inventory is invalid")
    if not exact_json_value(report.get("forbidden_mod_ids_loaded"), []):
        raise V7ContractError("The loaded forbidden mod intersection is not empty")
    expected_mods = {
    ...
    if not exact_json_value(report.get("mods"), expected_mods):
        raise V7ContractError("The server probe mod subset changed")
    if not exact_json_value(report.get("registry"), {
        ...
        raise V7ContractError("The server probe registry result changed")
    if not exact_json_value(report.get("enchantments"), {
        ...
        raise V7ContractError("The server probe enchantment result changed")
    if not exact_json_value(report.get("loot_condition"), {
        ...
        raise V7ContractError("The server probe loot-condition result changed")
    ...
    if not exact_json_value(report.get("ether_sources"), {
        ...
        raise V7ContractError("The server probe Ether-source result changed")
    ...
    if not isinstance(reload, dict) or set(reload) != {
        ...
        raise V7ContractError("The server probe reload result changed")
    ...
    if not exact_json_value(report.get("tags"), {
        ...
        raise V7ContractError("The server probe tag result changed")
    if not exact_json_value(report.get("lifecycle"), list(EXPECTED_LIFECYCLE)):
        raise V7ContractError("The server probe lifecycle changed")

    assertions = report.get("assertions")
    if not isinstance(assertions, list) or len(assertions) != len(
        EXPECTED_ASSERTION_NAMES
    ):
        raise V7ContractError("The server probe assertion inventory changed")
