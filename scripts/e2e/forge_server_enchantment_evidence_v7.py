#!/usr/bin/env python3
"""Validate live or archived Forge dedicated-server enchantment-registry evidence."""
...
PROFILE_MANIFEST_RELATIVE_PATH = Path(contract_v7.PROFILE_MANIFEST_RELATIVE_PATH)
...
ARCHIVE_RELATIVE_PATH = Path(
    "docs/evidence/forge-1.20.1/enchantment-registry-server-v7"
)
DEFAULT_RUNTIME_ROOT = RUNTIMES_ROOT / PROFILE_ID
DEFAULT_ARCHIVE_ROOT = REPOSITORY_ROOT / ARCHIVE_RELATIVE_PATH
ARCHIVE_DIRECTORY_NAME = "enchantment-registry-server-v7"
...
EXPECTED_ASSERTIONS = tuple(
    zip(
        contract_v7.EXPECTED_ASSERTION_NAMES,
        contract_v7.EXPECTED_ASSERTION_VALUES,
        strict=True,
    )
)
PROBE_LOG_PHASES = (
    *contract_v7.PROBE_LOG_PHASES,
    "loom_userdev_exit_scheduled",
)
TERMINATION_LOG_TOKEN = contract_v7.SERVER_LOG_TOKENS[-1]
...
def load_profile_manifest(path: Path = PROFILE_MANIFEST_PATH) -> ProfileRecord:
    ...
    if (
        record.size != contract_v7.PROFILE_MANIFEST_SIZE
        or record.sha256 != contract_v7.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The Forge server profile bytes differ from the immutable v7 contract"
        )
    return record
...
def validate_report(report: dict[str, object]) -> None:
    """Validates the report through the immutable profile-v7 contract."""
    try:
        ...
        raise EvidenceError(
            f"The Forge server report violates the v7 contract: {exception}"
        ) from exception
...
def validate_archive_root_identity(archive_root: Path) -> None:
    """Requires the immutable enchantment-registry server-v7 identity."""
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise EvidenceError(
            "The Forge server archive directory does not identify enchantment-registry profile v7"
        )
...
def validate_archive_manifest_shape(
    ...
    if (
        profile_record.size != contract_v7.PROFILE_MANIFEST_SIZE
        or profile_record.sha256 != contract_v7.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The archived Forge server profile differs from the immutable v7 contract"
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Validate Forge 1.20.1 dedicated-server enchantment-registry evidence."
        )
    )
