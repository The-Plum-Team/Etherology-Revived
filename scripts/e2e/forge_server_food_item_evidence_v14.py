#!/usr/bin/env python3
"""Validate live or archived Forge dedicated-server food-item evidence."""
...
PROFILE_MANIFEST_RELATIVE_PATH = Path(contract_v14.PROFILE_MANIFEST_RELATIVE_PATH)
...
ARCHIVE_RELATIVE_PATH = Path(
    "docs/evidence/forge-1.20.1/food-item-registry-server-v14"
)
DEFAULT_RUNTIME_ROOT = RUNTIMES_ROOT / PROFILE_ID
DEFAULT_ARCHIVE_ROOT = REPOSITORY_ROOT / ARCHIVE_RELATIVE_PATH
ARCHIVE_DIRECTORY_NAME = "food-item-registry-server-v14"
FOOD_ITEM_ARCHIVE_DIRECTORY_PATTERN = re.compile(
    r"food-item-registry-server-v[0-9]+"
)
...
EXPECTED_ASSERTIONS = tuple(
    zip(
        contract_v14.EXPECTED_ASSERTION_NAMES,
        contract_v14.EXPECTED_ASSERTION_VALUES,
        strict=True,
    )
)
PROBE_LOG_PHASES = (
    *contract_v14.PROBE_LOG_PHASES,
    "loom_userdev_exit_scheduled",
)
TERMINATION_LOG_TOKEN = contract_v14.SERVER_LOG_TOKENS[-1]
...
def load_profile_manifest(path: Path = PROFILE_MANIFEST_PATH) -> ProfileRecord:
    ...
    if (
        record.size != contract_v14.PROFILE_MANIFEST_SIZE
        or record.sha256 != contract_v14.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The Forge server profile bytes differ from the immutable v14 contract"
        )
    return record
...
def validate_report(report: dict[str, object]) -> None:
    """Validates the report through the immutable profile-v14 contract."""
    try:
        ...
        raise EvidenceError(
            f"The Forge server report violates the v14 contract: {exception}"
        ) from exception
...
def validate_archive_root_identity(archive_root: Path) -> None:
    """Requires the immutable food-item server-v14 identity."""
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise EvidenceError(
            "The Forge server archive directory does not identify food-item profile v14"
        )


def validate_no_competing_food_item_archives(repository_root: Path) -> None:
    """Rejects older or newer food-item evidence beside v14."""
    ...
    competing_directories = sorted(
        ...
        and FOOD_ITEM_ARCHIVE_DIRECTORY_PATTERN.fullmatch(entry.name)
        and (entry.is_dir() or entry.is_symlink())
    )
    if competing_directories:
        raise EvidenceError(
            "The Forge server archive has competing food-item evidence: "
...
def validate_archive_destination(
    ...
    validate_no_competing_food_item_archives(repository_root)
    return repository_root
...
def validate_archive_manifest_shape(
    ...
    if (
        profile_record.size != contract_v14.PROFILE_MANIFEST_SIZE
        or profile_record.sha256 != contract_v14.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The archived Forge server profile differs from the immutable v14 contract"
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Validate Forge 1.20.1 dedicated-server food-item evidence."
        )
    )
