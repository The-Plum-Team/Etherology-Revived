...
PROFILE_MANIFEST_RELATIVE_PATH = Path(contract_v16.PROFILE_MANIFEST_RELATIVE_PATH)
...
ARCHIVE_DIRECTORY_NAME = "forest-lantern-server-v16"
...
EXPECTED_ASSERTIONS = tuple(
    zip(
        contract_v16.EXPECTED_ASSERTION_NAMES,
        contract_v16.EXPECTED_ASSERTION_VALUES,
        strict=True,
    )
)
PROBE_LOG_PHASES = (
    *contract_v16.PROBE_LOG_PHASES,
    "loom_userdev_exit_scheduled",
)
TERMINATION_LOG_TOKEN = contract_v16.SERVER_LOG_TOKENS[-1]
...
def load_profile_manifest(path: Path = PROFILE_MANIFEST_PATH) -> ProfileRecord:
    ...
    if (
        record.size != contract_v16.PROFILE_MANIFEST_SIZE
        or record.sha256 != contract_v16.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The Forge server profile bytes differ from the immutable v16 contract"
        )
    return record
...
def validate_report(report: dict[str, object]) -> None:
    """Validates the report through the immutable profile-v16 contract."""
    try:
        ...
        raise EvidenceError(
            f"The Forge server report violates the v16 contract: {exception}"
        ) from exception
...
def validate_archive_manifest_shape(
    ...
    if (
        profile_record.size != contract_v16.PROFILE_MANIFEST_SIZE
        or profile_record.sha256 != contract_v16.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The archived Forge server profile differs from the immutable v16 contract"
        )
