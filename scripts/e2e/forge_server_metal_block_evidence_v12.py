#!/usr/bin/env python3
"""Validate live or archived Forge dedicated-server metal-block evidence."""
...
PROFILE_MANIFEST_RELATIVE_PATH = Path(contract_v12.PROFILE_MANIFEST_RELATIVE_PATH)
PROFILE_MANIFEST_PATH = REPOSITORY_ROOT / PROFILE_MANIFEST_RELATIVE_PATH
PROFILE_ID = contract_v12.PROFILE_ID
SCENARIO_ID = contract_v12.SCENARIO_ID
TASK_PATH = contract_v12.TASK_PATH
...
ARCHIVE_RELATIVE_PATH = Path(
    "docs/evidence/forge-1.20.1/metal-block-registry-server-v12"
)
DEFAULT_RUNTIME_ROOT = RUNTIMES_ROOT / PROFILE_ID
DEFAULT_ARCHIVE_ROOT = REPOSITORY_ROOT / ARCHIVE_RELATIVE_PATH
ARCHIVE_DIRECTORY_NAME = "metal-block-registry-server-v12"
METAL_BLOCK_ARCHIVE_DIRECTORY_PATTERN = re.compile(
    r"metal-block-registry-server-v[0-9]+"
)
...
REQUIRED_MOD_IDS = contract_v12.REQUIRED_MOD_IDS
FORBIDDEN_MOD_IDS = contract_v12.FORBIDDEN_MOD_IDS
EXPECTED_ASSERTIONS = tuple(
    zip(
        contract_v12.EXPECTED_ASSERTION_NAMES,
        contract_v12.EXPECTED_ASSERTION_VALUES,
        strict=True,
    )
)
PROBE_LOG_PHASES = (
    *contract_v12.PROBE_LOG_PHASES,
    "loom_userdev_exit_scheduled",
)
TERMINATION_LOG_TOKEN = contract_v12.SERVER_LOG_TOKENS[-1]
...
CLIENT_LOG_MARKERS = contract_v12.CLIENT_LOG_MARKERS
CLIENT_CLASS_PATTERN = contract_v12.CLIENT_CLASS_PATTERN
ALLOWED_DEDICATED_SERVER_CLIENT_CLASSES = (
    contract_v12.ALLOWED_DEDICATED_SERVER_CLIENT_CLASSES
)
PROFILE_FIELDS = {
    "schema",
    "profile",
...
def load_profile_manifest(path: Path = PROFILE_MANIFEST_PATH) -> ProfileRecord:
    ...
    if (
        record.size != contract_v12.PROFILE_MANIFEST_SIZE
        or record.sha256 != contract_v12.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The Forge server profile bytes differ from the immutable v12 contract"
        )
    return record
...
def validate_report(report: dict[str, object]) -> None:
    """Validates the report through the immutable profile-v12 contract."""
    try:
        contract_v12.validate_probe_report(
            report,
            list(REQUIRED_MOD_IDS),
            list(FORBIDDEN_MOD_IDS),
        )
    except contract_v12.V12ContractError as exception:
        raise EvidenceError(
            f"The Forge server report violates the v12 contract: {exception}"
        ) from exception
...
def validate_archive_root_identity(archive_root: Path) -> None:
    """Requires the immutable metal-block server-v12 identity."""
    if archive_root.name != ARCHIVE_DIRECTORY_NAME:
        raise EvidenceError(
            "The Forge server archive directory does not identify metal-block profile v12"
        )


def validate_no_competing_metal_block_archives(repository_root: Path) -> None:
    """Rejects older or newer metal-block evidence beside v12."""
    ...
    competing_directories = sorted(
        ...
        and METAL_BLOCK_ARCHIVE_DIRECTORY_PATTERN.fullmatch(entry.name)
        and (entry.is_dir() or entry.is_symlink())
    )
    if competing_directories:
        raise EvidenceError(
            "The Forge server archive has competing metal-block evidence: "
...
def validate_archive_destination(
    ...
    validate_no_competing_metal_block_archives(repository_root)
    return repository_root
...
def validate_archive_manifest_shape(
    ...
    if (
        profile_record.size != contract_v12.PROFILE_MANIFEST_SIZE
        or profile_record.sha256 != contract_v12.PROFILE_MANIFEST_SHA256
    ):
        raise EvidenceError(
            "The archived Forge server profile differs from the immutable v12 contract"
        )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Validate Forge 1.20.1 dedicated-server metal-block evidence."
        )
    )
