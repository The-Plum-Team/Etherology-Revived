...
def validate_active_profile(configuration: forge_client.ResolvedConfiguration) -> None:
    """Requires active Forge v18 profile bytes and runtime metadata."""
...
def validate_live_evidence(
    ...
    """Validates the exact fresh v18 runtime without mutating or launching it."""
...
def build_archive_manifest(
    ...
    """Builds an immutable seal from the exact live v18 capture."""
...
def write_archive_manifest(
    ...
    """Atomically seals and revalidates the copied v18 payload."""
...
def validate_archive_manifest(
    ...
    """Validates the v18 archive seal and payload records."""
...
def validate_archived_evidence(
    ...
    """Validates a sealed v18 archive without consulting live runtime state."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    operation.add_argument(
        "--live", action="store_true", help="validate the owned fresh v18 runtime"
    )
    operation.add_argument(
        "--archive", type=Path, help="validate one frozen v18 archive"
    )
    operation.add_argument(
        "--create-archive-manifest",
        type=Path,
        metavar="ARCHIVE",
        help="seal copied v18 payload from one explicit owned runtime",
    )
    parser.add_argument(
        "--capture-runtime",
        type=Path,
    ...
    parser.add_argument(
        "--profile-manifest",
        type=Path,
        help="v18 manifest used only while sealing",
    )
