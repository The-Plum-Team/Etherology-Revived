...
import client
import evidence
import fabric_attrahite_evidence_v28 as historical_attrahite_evidence
import fabric_attrahite_evidence_v29 as attrahite_evidence
from test_evidence import rgb_png


REPOSITORY_ROOT = SCRIPT_DIRECTORY.parent.parent
HISTORICAL_V28_STATE_RELATIVE_PATH = Path("scripts/e2e/.state")
HISTORICAL_V28_RUNTIME_RELATIVE_PATH = (
    HISTORICAL_V28_STATE_RELATIVE_PATH
    / "runtimes/etherology-e2e-fabric-1.20.1-v28"
)
HISTORICAL_V28_HARNESS_SIZE = 296_299
HISTORICAL_V28_HARNESS_SHA256 = (
    "0abc2b77b0090259510f536b0cd8f3f547eab8f2c00b71c7353b28d6947c73ee"
)
HISTORICAL_V28_FAILURE_HISTORY = {
    (
        HISTORICAL_V28_STATE_RELATIVE_PATH
        / "etherology-e2e-fabric-1.20.1-v28-start.attempted"
    ): (
        88,
        "c8b5f90e12904c271820f887d8a88f207e3abf2eaa0191f3b8b5e5ce86c9257f",
    ),
    (
        HISTORICAL_V28_RUNTIME_RELATIVE_PATH
        ...
        31_518,
        "68e6d0e3ec493bbda2c5133639f22373ec66814e8d99c4df7272209bbc92d7eb",
    ),
    (
        HISTORICAL_V28_RUNTIME_RELATIVE_PATH
    ...
    (
        HISTORICAL_V28_STATE_RELATIVE_PATH
        / "logs/fabric-1.20.1-20260901T020906Z.log"
    ): (
        15_353,
        "e88a7b1e95bc50416e0f404fe83ab1b7cc37e9f816b815d6ba077dfb1d772426",
    ),
    HISTORICAL_V28_RUNTIME_RELATIVE_PATH / "game/logs/latest.log": (
        14_916,
        "d43f7d35f18ea7e3d4bc8132669d28e7ccef3d86c5beb4317e22aa6ca74786c1",
    ),
    HISTORICAL_V28_RUNTIME_RELATIVE_PATH / "game/mods/etherology-e2e-harness.jar": (
        HISTORICAL_V28_HARNESS_SIZE,
        HISTORICAL_V28_HARNESS_SHA256,
    ),
}


def validate_consumed_v28_failure_history(repository_root: Path) -> None:
    state_root = repository_root / HISTORICAL_V28_STATE_RELATIVE_PATH
    runtime = repository_root / HISTORICAL_V28_RUNTIME_RELATIVE_PATH
    controller_logs = state_root / "logs"
    runtime_parent = state_root / "runtimes"
    resolved = {
        repository_root / relative_path: contract
        for relative_path, contract in HISTORICAL_V28_FAILURE_HISTORY.items()
    }

    for directory in (state_root, runtime_parent, controller_logs):
        if directory.is_symlink():
            raise AssertionError(
                f"The consumed v28 failure-history parent is linked: {directory}"
            )
        if directory.exists() and not directory.is_dir():
            raise AssertionError(
                f"The consumed v28 failure-history parent is not a directory: {directory}"
            )
    ...
    for directory in sorted(
        ...
        if not directory.is_dir():
            raise AssertionError(
                f"The consumed v28 failure-history parent is missing: {directory}"
            )
        if directory.is_symlink():
            raise AssertionError(
                f"The consumed v28 failure-history parent is linked: {directory}"
    ...
    if not all(present):
        raise AssertionError("The consumed v28 failure history is partial")
    for path, (expected_size, expected_sha256) in resolved.items():
        if not path.is_file() or path.is_symlink():
            raise AssertionError(
                f"The consumed v28 failure-history artifact is missing or linked: {path}"
            )
        if path.stat().st_size != expected_size:
            raise AssertionError(
                f"The consumed v28 failure-history artifact size changed: {path}"
            )
        if evidence.sha256_file(path) != expected_sha256:
            raise AssertionError(
                f"The consumed v28 failure-history artifact bytes changed: {path}"
            )
...
def build_report(scenario_root: Path) -> dict[str, object]:
    ...
    report: dict[str, object] = {
        ...
        "attrahite": {
            ...
            "latest_client_fixtures": dict.fromkeys(
                attrahite_evidence.PHASES,
                attrahite_evidence.EXPECTED_SNAPSHOT,
            ),
        },
    }
...
class ActiveProfileTests(unittest.TestCase):
    def test_active_profile_is_exact_v29_snapshot(self) -> None:
        ...
        self.assertEqual(
            ...
            (SCRIPT_DIRECTORY / "fabric-1.20.1-profile-v29.json").read_bytes(),
        )

    def test_v20_through_v28_profiles_remain_byte_exact(self) -> None:
        expected = {
            ...
            27: "b2ea801d26457b7b3ed807d04997cf76e8eaf2c10ca536231d29858a5ce323da",
            28: "34e187dae9cff37754238ce41b4dc707265d1d083df0a4f133b84b6780fc98e1",
        }
    ...
    def test_historical_v28_verifier_rejects_active_v29_profile(self) -> None:
        configuration = client.load_configuration()
    ...
    def test_historical_v28_harness_pin_remains_explicit(self) -> None:
        self.assertEqual(
            HISTORICAL_V28_HARNESS_SIZE,
    ...
    def test_v25_through_v28_verifier_histories_remain_byte_exact(self) -> None:
        expected = {
            ...
            "fabric_attrahite_evidence_v27.py": (
                "171f2847ece854afc356e984145f43b0bcc6286f6a3db2d4154be24c905ca523"
            ),
            "test_fabric_attrahite_evidence_v27.py": (
                "d7fc25fd1aac3f58ce7c8c373b4cc59b2c40e0d27332f2bafacb62df2f5b54b8"
            ),
            "fabric_attrahite_evidence_v28.py": (
                "c5e2881f44d603b0c4e4b4041f5246b34e13b2caf79a3ad73cd9230e99b53deb"
            ),
            "test_fabric_attrahite_evidence_v28.py": (
                "fbc042a7d6b18e94a6b149385423f7bda740e91b95aab0d004962fa00512a430"
            ),
        }
    ...
    def test_consumed_v28_failure_history_remains_byte_exact_when_present(
        self,
    ) -> None:
        validate_consumed_v28_failure_history(REPOSITORY_ROOT)

    def test_consumed_v28_history_rejects_dangling_leaf_symlinks(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            repository_root = Path(temporary_directory)
            missing_targets = repository_root / "missing-targets"
            for index, relative_path in enumerate(HISTORICAL_V28_FAILURE_HISTORY):
    ...
    def test_consumed_v28_history_rejects_linked_parent_components(self) -> None:
        for parent_name in ("runtimes", "logs"):
            with self.subTest(parent_name=parent_name):
                with tempfile.TemporaryDirectory() as temporary_directory:
                    repository_root = Path(temporary_directory)
                    state_root = (
                        repository_root / HISTORICAL_V28_STATE_RELATIVE_PATH
                    )
    ...
    def test_consumed_v28_failure_fingerprint_is_preserved_when_present(self) -> None:
        report_path = (
            REPOSITORY_ROOT
        ...
        self.assertEqual(6_144, report["client_ticks"])
        self.assertEqual(
        ...
        initial = report["attrahite"]["lighting"]["initial"]
        self.assertEqual(20, initial["server_stable_ticks"])
        self.assertEqual(0, initial["client_stable_ticks"])
        self.assertEqual(attrahite_evidence.EXPECTED_SKY_LIGHT, initial["server_sky"])
        self.assertEqual(
            attrahite_evidence.EXPECTED_BLOCK_LIGHT,
            initial["server_block"],
        )
        self.assertEqual(
            dict.fromkeys(attrahite_evidence.LIGHT_SAMPLE_POSITIONS, 0),
            initial["client_sky"],
        )
        self.assertEqual(initial["client_sky"], initial["client_block"])
    ...
    def test_assertion_inventory_matches_harness_contract(self) -> None:
        self.assertEqual(90, len(attrahite_evidence.EXPECTED_ASSERTION_NAMES))
        self.assertEqual(
            "isolated_save_directory_present",
            attrahite_evidence.EXPECTED_ASSERTION_NAMES[-1],
        )

    def test_authoritative_v28_light_samples_define_v29_readiness(self) -> None:
        self.assertEqual(
            {
                "0,121,-8": 15,
                "0,121,0": 15,
...
class ReportContractTests(unittest.TestCase):
    ...
    def test_rejects_world_ready_resource_and_mechanics_drift(self) -> None:
        mutations = (
            ...
            lambda report: report["attrahite"]["latest_client_fixtures"].__setitem__(
                "initial", "missing"
            ),
        )
        for mutate in mutations:
...
class ArchiveContractTests(unittest.TestCase):
    ...
    def test_accepts_exact_self_contained_archive(self) -> None:
        ...
        self.assertEqual(90, summary.assertion_count)
    ...
    def test_rejects_extra_payload_and_competing_versioned_archive(self) -> None:
        ...
        competing = self.parent / "attrahite-block-registry-v28"
        competing.mkdir()
        with self.assertRaises(client.E2EError):
            attrahite_evidence.validate_archived_evidence(self.archive_root)
    ...
    def test_rejects_missing_live_runtime_fail_closed(self) -> None:
        configuration = client.load_configuration()
        missing = self.parent / "missing-v29-runtime"

        with self.assertRaises(client.E2EError):
            attrahite_evidence.validate_live_evidence(configuration, missing)
