...
HISTORICAL_V29_STATE_RELATIVE_PATH = Path("scripts/e2e/.state")
HISTORICAL_V29_RUNTIME_RELATIVE_PATH = (
    HISTORICAL_V29_STATE_RELATIVE_PATH
    / "runtimes/etherology-e2e-fabric-1.20.1-v29"
)
HISTORICAL_V29_HARNESS_SIZE = 296_850
...
HISTORICAL_V29_FAILURE_HISTORY = {
    (
        HISTORICAL_V29_STATE_RELATIVE_PATH
        / "etherology-e2e-fabric-1.20.1-v29-start.attempted"
    ): (
        88,
        "2db1ebbf146847e9e19b898ddc8be4cc45f971c31b7beb4c71ca7b2e911028a9",
    ),
    (
        HISTORICAL_V29_RUNTIME_RELATIVE_PATH
        / "evidence/attrahite-block-registry/reports/report.json"
        ...
        32_814,
        "fbbcd658a79f1a294527c8bdfc23727724dd335ce7dd5e1bad6aa0369b7fe676",
    ),
    (
        HISTORICAL_V29_RUNTIME_RELATIVE_PATH
        / "evidence/attrahite-block-registry/reports/done.marker"
    ...
    (
        HISTORICAL_V29_STATE_RELATIVE_PATH
        / "logs/fabric-1.20.1-20260901T033904Z.log"
    ): (
        15_285,
        "bb2c8ae5eba9858a9bb8235d965e2779c961c0cf0b0bbb93d4c46c7b6e1ad732",
    ),
    HISTORICAL_V29_RUNTIME_RELATIVE_PATH / "game/logs/latest.log": (
        14_848,
        "830dd89c0a9ec509f38889341413186eb1dc4c13b5a991f2f45f4de37283f6db",
    ),
    HISTORICAL_V29_RUNTIME_RELATIVE_PATH / "game/mods/etherology-e2e-harness.jar": (
        HISTORICAL_V29_HARNESS_SIZE,
        HISTORICAL_V29_HARNESS_SHA256,
    ),
}
...
class ActiveProfileTests(unittest.TestCase):
    ...
    def test_v20_through_v29_profiles_remain_byte_exact(self) -> None:
        expected = {
            ...
            29: "71c6296a6959689dfcce50d3f3965107bd8e6cf9a447c3de9d06f2c7da80de12",
        }
    ...
    def test_historical_v29_harness_pin_remains_explicit(self) -> None:
        self.assertEqual(
            HISTORICAL_V29_HARNESS_SIZE,
    ...
    def test_v25_through_v29_verifier_histories_remain_byte_exact(self) -> None:
        expected = {
            ...
            "fabric_attrahite_evidence_v29.py": (
                "adaa6fcf19e94fb43af20544c6d61c359feaef355c41d8ab3cc45f279ecb351e"
            ),
            "test_fabric_attrahite_evidence_v29.py": (
                "f8a3780d71dab2254bb153baa4cdb22737529122f93167ff996787c599b834f2"
            ),
        }
    ...
    def test_consumed_v29_failure_history_remains_byte_exact_when_present(
        self,
    ) -> None:
        validate_consumed_v29_failure_history(REPOSITORY_ROOT)

    def test_consumed_v29_history_rejects_dangling_leaf_symlinks(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            repository_root = Path(temporary_directory)
            missing_targets = repository_root / "missing-targets"
            for index, relative_path in enumerate(HISTORICAL_V29_FAILURE_HISTORY):
                artifact = repository_root / relative_path
                artifact.parent.mkdir(parents=True, exist_ok=True)
                artifact.symlink_to(missing_targets / str(index))
    ...
    def test_consumed_v29_history_rejects_linked_parent_components(self) -> None:
        for parent_name in ("runtimes", "logs"):
            with self.subTest(parent_name=parent_name):
                with tempfile.TemporaryDirectory() as temporary_directory:
                    repository_root = Path(temporary_directory)
                    state_root = (
                        repository_root / HISTORICAL_V29_STATE_RELATIVE_PATH
                    )
                    state_root.mkdir(parents=True)
                    linked_target = repository_root / f"linked-{parent_name}"
                    linked_target.mkdir()
    ...
    def test_consumed_v29_failure_fingerprint_is_preserved_when_present(self) -> None:
        report_path = (
            REPOSITORY_ROOT
        ...
        self.assertEqual(6_155, report["client_ticks"])
        self.assertEqual(
            "Timed out in WAITING_FOR_CLIENT_MIRROR after 6000 client ticks",
            report["lifecycle_failure"],
        )
        self.assertEqual(90, len(report["assertions"]))
        self.assertEqual(73, sum(row["passed"] for row in report["assertions"]))
    ...
    def test_authoritative_v29_light_samples_define_v30_readiness(self) -> None:
        self.assertEqual(
            {
                "0,121,-8": 15,
                "0,121,0": 15,
...
class ArchiveContractTests(unittest.TestCase):
    ...
    def test_rejects_extra_payload_and_competing_versioned_archive(self) -> None:
        ...
        competing = self.parent / "attrahite-block-registry-v29"
        competing.mkdir()
        with self.assertRaises(client.E2EError):
            attrahite_evidence.validate_archived_evidence(self.archive_root)
    ...
    def test_rejects_missing_live_runtime_fail_closed(self) -> None:
        configuration = client.load_configuration()
        missing = self.parent / "missing-v30-runtime"
