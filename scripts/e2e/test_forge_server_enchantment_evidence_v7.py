...
import forge_server_enchantment_evidence_v7 as evidence
...
def valid_report() -> dict[str, object]:
    ...
    enabled_data_pack_names = sorted(
        ...
        + [contract_v7.RELOAD_PACK_ENABLED_NAME]
    )
    return {
        "schema": 5,
        ...
        "enchantments": {
            "registry_id": contract_v7.ENCHANTMENT_REGISTRY_ID,
            "non_treasure_tag_id": contract_v7.NON_TREASURE_TAG_ID,
            "etherology_enchantment_ids": list(contract_v7.ENCHANTMENT_IDS),
            "peal": copy.deepcopy(contract_v7.ENCHANTMENTS["peal"]),
            "reflection": copy.deepcopy(
                contract_v7.ENCHANTMENTS["reflection"]
            ),
            "non_treasure_etherology_enchantment_ids": list(
                contract_v7.ENCHANTMENT_IDS
            ),
        ...
        "ether_sources": {
            "listener_class": contract_v7.ETHER_SOURCE_LISTENER_CLASS,
            "resource_directory": "ether_sources",
            "initial": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v7.INITIAL_ETHER_SOURCE_ENTRIES
                ),
            },
            "server_started": {
                "capture_error": "",
            ...
            "reloaded": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v7.RELOADED_ETHER_SOURCE_ENTRIES
                ),
            },
        ...
        "reload": {
            "pack_directory": contract_v7.RELOAD_PACK_DIRECTORY,
            "pack_resources": list(contract_v7.RELOAD_PACK_RESOURCES),
            "enabled_pack_name": contract_v7.RELOAD_PACK_ENABLED_NAME,
        ...
        "lifecycle": list(contract_v7.EXPECTED_LIFECYCLE),
...
def build_fixture(root: Path) -> Fixture:
    ...
    profile_snapshot = (
        evidence.REPOSITORY_ROOT / contract_v7.PROFILE_SNAPSHOT_RELATIVE_PATH
    )
...
class ContractOwnershipTests(unittest.TestCase):
    def test_v7_archive_contract_does_not_import_the_mutable_runner(self) -> None:
        evidence_source = Path(evidence.__file__).read_text(encoding="utf-8")
        contract_source = Path(contract_v7.__file__).read_text(encoding="utf-8")
        ...
        self.assertEqual(95, len(contract_v7.EXPECTED_ASSERTION_NAMES))
        self.assertEqual(95, len(contract_v7.EXPECTED_ASSERTION_VALUES))
        snapshot = (
            evidence.REPOSITORY_ROOT / contract_v7.PROFILE_SNAPSHOT_RELATIVE_PATH
        )
        self.assertEqual(
            contract_v7.PROFILE_MANIFEST_SIZE,
...
class LiveEvidenceTests(unittest.TestCase):
    def test_exact_live_runtime_passes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(95, summary.assertion_count)
            self.assertEqual(
    ...
    def test_registry_and_enchantment_contracts_are_exact(self) -> None:
        cases = (
        ...
        for field, replacement in cases:
            with self.subTest(field=field), tempfile.TemporaryDirectory() as temporary:
                ...
                with self.assertRaisesRegex(evidence.EvidenceError, "v7 contract"):
                    evidence.validate_live_runtime(
                        fixture.runtime,
                        fixture.profile,
                        expected_runtime=fixture.runtime,
    ...
    def test_reload_command_pack_and_stability_contract_are_exact(self) -> None:
        cases = (
            ...
            (
                "pack resources",
                "pack_resources",
                list(reversed(contract_v7.RELOAD_PACK_RESOURCES)),
            ),
    ...
    def test_all_95_assertions_must_pass_in_exact_order_and_value(self) -> None:
        mutations = (
...
class ArchiveEvidenceTests(unittest.TestCase):
    ...
    def test_resealed_v7_reload_report_tampering_is_rejected(self) -> None:
        cases = (
            (
                "schema downgrade",
                lambda report: report.__setitem__("schema", 2),
            ...
            (
                ...
                lambda report: report["reload"][
                    "enabled_data_pack_names"
                ].remove(contract_v7.RELOAD_PACK_ENABLED_NAME),
            ),
            (
    ...
    def test_resealed_v7_reload_log_tampering_is_rejected(self) -> None:
        cases = (
            (
    ...
    def test_archive_requires_exact_destination_and_enchantment_registry_v7_name(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            fixture = build_fixture(Path(temporary_directory))
            ...
            with self.assertRaisesRegex(
                evidence.EvidenceError,
                "repository path|enchantment-registry profile v7",
            ):
                evidence.validate_archived_evidence(
                    wrong_name,
