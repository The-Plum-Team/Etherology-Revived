...
import forge_server_material_item_evidence_v11 as evidence
...
def valid_report() -> dict[str, object]:
    ...
    enabled_data_pack_names = sorted(
        ...
        + [contract_v11.RELOAD_PACK_ENABLED_NAME]
    )
    return {
        "schema": 7,
        ...
        "enchantments": {
            "registry_id": contract_v11.ENCHANTMENT_REGISTRY_ID,
            "non_treasure_tag_id": contract_v11.NON_TREASURE_TAG_ID,
            "etherology_enchantment_ids": list(contract_v11.ENCHANTMENT_IDS),
            "peal": copy.deepcopy(contract_v11.ENCHANTMENTS["peal"]),
            "reflection": copy.deepcopy(
                contract_v11.ENCHANTMENTS["reflection"]
            ),
            "non_treasure_etherology_enchantment_ids": list(
                contract_v11.ENCHANTMENT_IDS
            ),
        ...
        "particles": {
            "registry_id": contract_v11.PARTICLE_REGISTRY_ID,
            "capture_error": "",
            "etherology_particle_ids": list(contract_v11.PARTICLE_IDS),
            "payload_families": list(
                contract_v11.PARTICLE_PAYLOAD_FAMILIES
            ),
            "entries": copy.deepcopy(contract_v11.PARTICLES),
            "seal_types": {
                "order": list(contract_v11.SEAL_TYPE_ORDER),
                "codec_round_trips_exact": True,
                "entries": copy.deepcopy(contract_v11.SEAL_TYPES),
            },
            "same_state_at_server_started": True,
            "registry_stable_after_reload": True,
        ...
        "material_items": {
            "registry_id": contract_v11.MATERIAL_ITEM_REGISTRY_ID,
            "capture_error": "",
            "material_item_ids": list(contract_v11.MATERIAL_ITEM_IDS),
            "vanilla_item_class": contract_v11.MATERIAL_ITEM_CLASS,
            "max_counts": contract_v11.MATERIAL_ITEM_CANONICAL_MAX_COUNTS,
            "save_representations": (
                contract_v11.MATERIAL_ITEM_CANONICAL_SAVE_REPRESENTATIONS
            ),
            "entries": copy.deepcopy(contract_v11.MATERIAL_ITEMS),
            "same_state_at_server_started": True,
            "registry_stable_after_reload": True,
            "properties_stable_after_reload": True,
        ...
        "ether_sources": {
            "listener_class": contract_v11.ETHER_SOURCE_LISTENER_CLASS,
            "resource_directory": "ether_sources",
            "initial": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v11.INITIAL_ETHER_SOURCE_ENTRIES
                ),
            },
            "server_started": {
                "capture_error": "",
            ...
            "reloaded": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v11.RELOADED_ETHER_SOURCE_ENTRIES
                ),
            },
        ...
        "reload": {
            "pack_directory": contract_v11.RELOAD_PACK_DIRECTORY,
            "pack_resources": list(contract_v11.RELOAD_PACK_RESOURCES),
            "enabled_pack_name": contract_v11.RELOAD_PACK_ENABLED_NAME,
        ...
        "lifecycle": list(contract_v11.EXPECTED_LIFECYCLE),
...
def build_fixture(root: Path) -> Fixture:
    ...
    profile_snapshot = (
        evidence.REPOSITORY_ROOT / contract_v11.PROFILE_SNAPSHOT_RELATIVE_PATH
    )
...
class ContractOwnershipTests(unittest.TestCase):
    def test_v11_archive_contract_does_not_import_the_mutable_runner(self) -> None:
        evidence_source = Path(evidence.__file__).read_text(encoding="utf-8")
        contract_source = Path(contract_v11.__file__).read_text(encoding="utf-8")
        ...
        self.assertEqual(163, len(contract_v11.EXPECTED_ASSERTION_NAMES))
        self.assertEqual(163, len(contract_v11.EXPECTED_ASSERTION_VALUES))
        self.assertEqual(
            Path(
        ...
        self.assertEqual(
            contract_v11.PROFILE_MANIFEST_SIZE,
        ...
        self.assertEqual(
            {"enum_name", "as_string"},
            set(contract_v11.SEAL_TYPES["empty"]),
        )
        for seal_name in ("keta", "rella", "via", "clos"):
            self.assertEqual(
                ...
                set(contract_v11.SEAL_TYPES[seal_name]),
            )
        self.assertEqual(14, len(contract_v11.MATERIAL_ITEM_IDS))
        self.assertEqual(
            16,
            contract_v11.MATERIAL_ITEM_MAX_COUNTS[
        ...
        self.assertTrue(
            all(
                ...
                for identifier, max_count in (
                    contract_v11.MATERIAL_ITEM_MAX_COUNTS.items()
                )
...
class LiveEvidenceTests(unittest.TestCase):
    def test_exact_live_runtime_passes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(163, summary.assertion_count)
            self.assertEqual(
    ...
    def test_registry_enchantment_and_particle_contracts_are_exact(self) -> None:
        ...
        for field, replacement in cases:
            with self.subTest(field=field), tempfile.TemporaryDirectory() as temporary:
                ...
                with self.assertRaisesRegex(evidence.EvidenceError, "v11 contract"):
                    evidence.validate_live_runtime(
                        fixture.runtime,
                        fixture.profile,
                        expected_runtime=fixture.runtime,
    ...
    def test_material_item_registry_stack_and_reload_contracts_are_exact(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            fixture = build_fixture(Path(temporary_directory))
            report_path = fixture.scenario / "reports" / "report.json"
            assertion_index = contract_v11.EXPECTED_ASSERTION_NAMES.index(
    ...
    def test_reload_command_pack_and_stability_contract_are_exact(self) -> None:
        cases = (
            ...
            (
                "pack resources",
                "pack_resources",
                list(reversed(contract_v11.RELOAD_PACK_RESOURCES)),
            ),
    ...
    def test_all_163_assertions_must_pass_in_exact_order_and_value(self) -> None:
        mutations = (
...
class ArchiveEvidenceTests(unittest.TestCase):
    ...
    def test_resealed_v11_reload_report_tampering_is_rejected(self) -> None:
        cases = (
            (
                "schema downgrade",
                lambda report: report.__setitem__("schema", 2),
            ...
            (
                ...
                lambda report: report["reload"][
                    "enabled_data_pack_names"
                ].remove(contract_v11.RELOAD_PACK_ENABLED_NAME),
            ),
            (
    ...
    def test_resealed_v11_reload_log_tampering_is_rejected(self) -> None:
        cases = (
            (
    ...
    def test_archive_requires_exact_destination_and_material_item_v11_name(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            fixture = build_fixture(Path(temporary_directory))
            ...
            with self.assertRaisesRegex(
                evidence.EvidenceError,
                "repository path|material-item profile v11",
            ):
                evidence.validate_archived_evidence(
                    wrong_name,
    ...
    def test_archive_rejects_older_and_newer_material_item_directories(
        self,
    ) -> None:
        for competing_name in (
            "material-item-registry-server-v10",
            "material-item-registry-server-v12",
        ):
            for operation in ("create", "validate"):
                with (
                    ...
                    with self.assertRaisesRegex(
                        evidence.EvidenceError,
                        "competing material-item evidence",
                    ):
                        if operation == "create":
                            evidence.write_archive_manifest(
                                fixture.profile,
