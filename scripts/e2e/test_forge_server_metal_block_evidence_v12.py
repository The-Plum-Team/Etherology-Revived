...
import forge_server_contract_v12 as contract_v12
import forge_server_metal_block_evidence_v12 as evidence
...
def valid_report() -> dict[str, object]:
    ...
    enabled_data_pack_names = sorted(
        ...
        + [contract_v12.RELOAD_PACK_ENABLED_NAME]
    )
    return {
        "schema": 8,
        ...
        "enchantments": {
            "registry_id": contract_v12.ENCHANTMENT_REGISTRY_ID,
            "non_treasure_tag_id": contract_v12.NON_TREASURE_TAG_ID,
            "etherology_enchantment_ids": list(contract_v12.ENCHANTMENT_IDS),
            "peal": copy.deepcopy(contract_v12.ENCHANTMENTS["peal"]),
            "reflection": copy.deepcopy(
                contract_v12.ENCHANTMENTS["reflection"]
            ),
            "non_treasure_etherology_enchantment_ids": list(
                contract_v12.ENCHANTMENT_IDS
            ),
        ...
        "particles": {
            "registry_id": contract_v12.PARTICLE_REGISTRY_ID,
            "capture_error": "",
            "etherology_particle_ids": list(contract_v12.PARTICLE_IDS),
            "payload_families": list(
                contract_v12.PARTICLE_PAYLOAD_FAMILIES
            ),
            "entries": copy.deepcopy(contract_v12.PARTICLES),
            "seal_types": {
                "order": list(contract_v12.SEAL_TYPE_ORDER),
                "codec_round_trips_exact": True,
                "entries": copy.deepcopy(contract_v12.SEAL_TYPES),
            },
            "same_state_at_server_started": True,
            "registry_stable_after_reload": True,
        ...
        "material_items": {
            "registry_id": contract_v12.MATERIAL_ITEM_REGISTRY_ID,
            "capture_error": "",
            "material_item_ids": list(contract_v12.MATERIAL_ITEM_IDS),
            "vanilla_item_class": contract_v12.MATERIAL_ITEM_CLASS,
            "max_counts": contract_v12.MATERIAL_ITEM_CANONICAL_MAX_COUNTS,
            "save_representations": (
                contract_v12.MATERIAL_ITEM_CANONICAL_SAVE_REPRESENTATIONS
            ),
            "entries": copy.deepcopy(contract_v12.MATERIAL_ITEMS),
            "same_state_at_server_started": True,
            "registry_stable_after_reload": True,
            "properties_stable_after_reload": True,
        ...
        "metal_blocks": {
            "block_registry_id": contract_v12.METAL_BLOCK_REGISTRY_ID,
            "item_registry_id": contract_v12.METAL_BLOCK_ITEM_REGISTRY_ID,
            "capture_error": "",
            "metal_block_ids": list(contract_v12.METAL_BLOCK_IDS),
            "metal_block_item_ids": list(contract_v12.METAL_BLOCK_IDS),
            "vanilla_block_class": contract_v12.METAL_BLOCK_CLASS,
            "block_item_class": contract_v12.BLOCK_ITEM_CLASS,
            "properties": contract_v12.METAL_BLOCK_CANONICAL_PROPERTIES,
            "save_representations": (
                contract_v12.METAL_BLOCK_CANONICAL_SAVE_REPRESENTATIONS
            ),
            "entries": copy.deepcopy(contract_v12.METAL_BLOCKS),
            "placement": {
                "capture_error": "",
                "positions": copy.deepcopy(
                    contract_v12.METAL_BLOCK_PLACEMENT_POSITIONS
                ),
                "placed_block_ids": {
                    identifier: identifier
                    for identifier in contract_v12.METAL_BLOCK_IDS
                },
                "exact": True,
                "stable_after_reload": True,
            },
        ...
        "ether_sources": {
            "listener_class": contract_v12.ETHER_SOURCE_LISTENER_CLASS,
            "resource_directory": "ether_sources",
            "initial": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v12.INITIAL_ETHER_SOURCE_ENTRIES
                ),
            },
            "server_started": {
                "capture_error": "",
            ...
            "reloaded": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v12.RELOADED_ETHER_SOURCE_ENTRIES
                ),
            },
        ...
        "reload": {
            "pack_directory": contract_v12.RELOAD_PACK_DIRECTORY,
            "pack_resources": list(contract_v12.RELOAD_PACK_RESOURCES),
            "enabled_pack_name": contract_v12.RELOAD_PACK_ENABLED_NAME,
        ...
        "lifecycle": list(contract_v12.EXPECTED_LIFECYCLE),
...
def build_fixture(root: Path) -> Fixture:
    ...
    profile_snapshot = (
        evidence.REPOSITORY_ROOT / contract_v12.PROFILE_SNAPSHOT_RELATIVE_PATH
    )
...
class ContractOwnershipTests(unittest.TestCase):
    def test_v12_archive_contract_does_not_import_the_mutable_runner(self) -> None:
        evidence_source = Path(evidence.__file__).read_text(encoding="utf-8")
        contract_source = Path(contract_v12.__file__).read_text(encoding="utf-8")
        ...
        self.assertEqual(188, len(contract_v12.EXPECTED_ASSERTION_NAMES))
        self.assertEqual(188, len(contract_v12.EXPECTED_ASSERTION_VALUES))
        self.assertEqual(
            Path(
                "docs/evidence/forge-1.20.1/"
                "metal-block-registry-server-v12"
            ),
        ...
        self.assertEqual(
            contract_v12.PROFILE_MANIFEST_SIZE,
            snapshot.stat().st_size,
        )
        self.assertEqual(
            contract_v12.PROFILE_MANIFEST_SHA256,
            evidence.sha256_file(snapshot),
        )
        self.assertEqual(
            {"enum_name", "as_string"},
            set(contract_v12.SEAL_TYPES["empty"]),
        )
        for seal_name in ("keta", "rella", "via", "clos"):
            self.assertEqual(
                ...
                set(contract_v12.SEAL_TYPES[seal_name]),
            )
        self.assertEqual(14, len(contract_v12.MATERIAL_ITEM_IDS))
        self.assertEqual(
            16,
            contract_v12.MATERIAL_ITEM_MAX_COUNTS[
        ...
        self.assertTrue(
            all(
                ...
                for identifier, max_count in (
                    contract_v12.MATERIAL_ITEM_MAX_COUNTS.items()
                )
        ...
        self.assertEqual(
            ...
            contract_v12.METAL_BLOCK_SPECS["etherology:azel_block"],
        )
        self.assertEqual(
            {
                "hardness": 5.0,
            ...
            contract_v12.METAL_BLOCK_SPECS["etherology:ebony_block"],
        )
        self.assertEqual(
            {
                "hardness": 3.0,
            ...
            contract_v12.METAL_BLOCK_SPECS["etherology:ethril_block"],
        )
        self.assertEqual(
            {
...
class LiveEvidenceTests(unittest.TestCase):
    def test_exact_live_runtime_passes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(188, summary.assertion_count)
            self.assertEqual(
    ...
    def test_registry_enchantment_and_particle_contracts_are_exact(self) -> None:
        ...
        for field, replacement in cases:
            with self.subTest(field=field), tempfile.TemporaryDirectory() as temporary:
                ...
                with self.assertRaisesRegex(evidence.EvidenceError, "v12 contract"):
                    evidence.validate_live_runtime(
                        fixture.runtime,
                        fixture.profile,
                        expected_runtime=fixture.runtime,
    ...
    def test_material_item_registry_stack_and_reload_contracts_are_exact(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            fixture = build_fixture(Path(temporary_directory))
            report_path = fixture.scenario / "reports" / "report.json"
            assertion_index = contract_v12.EXPECTED_ASSERTION_NAMES.index(
    ...
    def test_reload_command_pack_and_stability_contract_are_exact(self) -> None:
        cases = (
            ...
            (
                "pack resources",
                "pack_resources",
                list(reversed(contract_v12.RELOAD_PACK_RESOURCES)),
            ),
    ...
    def test_all_188_assertions_must_pass_in_exact_order_and_value(self) -> None:
        mutations = (
...
class ArchiveEvidenceTests(unittest.TestCase):
    ...
    def test_resealed_v12_reload_report_tampering_is_rejected(self) -> None:
        cases = (
            (
                "schema downgrade",
                lambda report: report.__setitem__("schema", 2),
            ...
            (
                ...
                lambda report: report["reload"][
                    "enabled_data_pack_names"
                ].remove(contract_v12.RELOAD_PACK_ENABLED_NAME),
            ),
            (
    ...
    def test_resealed_v12_reload_log_tampering_is_rejected(self) -> None:
        cases = (
            (
    ...
    def test_archive_requires_exact_destination_and_metal_block_v12_name(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            fixture = build_fixture(Path(temporary_directory))
            ...
            with self.assertRaisesRegex(
                evidence.EvidenceError,
                "repository path|metal-block profile v12",
            ):
                evidence.validate_archived_evidence(
                    wrong_name,
    ...
    def test_archive_rejects_older_and_newer_metal_block_directories(
        self,
    ) -> None:
        for competing_name in (
            "metal-block-registry-server-v11",
            ...
            for operation in ("create", "validate"):
                with (
                    ...
                    with self.assertRaisesRegex(
                        evidence.EvidenceError,
                        "competing metal-block evidence",
                    ):
                        if operation == "create":
                            evidence.write_archive_manifest(
                                fixture.profile,
