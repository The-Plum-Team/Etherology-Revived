...
import forge_server_reload_evidence_v6 as evidence
...
def valid_report() -> dict[str, object]:
    ...
    enabled_data_pack_names = sorted(
        ...
        + [contract_v6.RELOAD_PACK_ENABLED_NAME]
    )
    return {
        "schema": 4,
        ...
        "ether_sources": {
            "listener_class": contract_v6.ETHER_SOURCE_LISTENER_CLASS,
            "resource_directory": "ether_sources",
            "initial": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v6.INITIAL_ETHER_SOURCE_ENTRIES
                ),
            },
            "server_started": {
                "capture_error": "",
            ...
            "reloaded": {
                "capture_error": "",
                "entries": copy.deepcopy(
                    contract_v6.RELOADED_ETHER_SOURCE_ENTRIES
                ),
            },
        ...
        "reload": {
            "pack_directory": contract_v6.RELOAD_PACK_DIRECTORY,
            "pack_resources": list(contract_v6.RELOAD_PACK_RESOURCES),
            "enabled_pack_name": contract_v6.RELOAD_PACK_ENABLED_NAME,
        ...
        "lifecycle": list(contract_v6.EXPECTED_LIFECYCLE),
...
def build_fixture(root: Path) -> Fixture:
    ...
    profile_snapshot = (
        evidence.REPOSITORY_ROOT / contract_v6.PROFILE_SNAPSHOT_RELATIVE_PATH
    )
...
class ContractOwnershipTests(unittest.TestCase):
    def test_v6_archive_contract_does_not_import_the_mutable_runner(self) -> None:
        evidence_source = Path(evidence.__file__).read_text(encoding="utf-8")
        contract_source = Path(contract_v6.__file__).read_text(encoding="utf-8")
        ...
        self.assertEqual(72, len(contract_v6.EXPECTED_ASSERTION_NAMES))
        self.assertEqual(72, len(contract_v6.EXPECTED_ASSERTION_VALUES))
        snapshot = (
            evidence.REPOSITORY_ROOT / contract_v6.PROFILE_SNAPSHOT_RELATIVE_PATH
        )
        self.assertEqual(
            contract_v6.PROFILE_MANIFEST_SIZE,
...
class LiveEvidenceTests(unittest.TestCase):
    def test_exact_live_runtime_passes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(72, summary.assertion_count)
            self.assertEqual(
    ...
    def test_registry_identity_and_singleton_namespace_are_exact(self) -> None:
        ...
        for field, replacement in cases:
            with self.subTest(field=field), tempfile.TemporaryDirectory() as temporary:
                ...
                with self.assertRaisesRegex(evidence.EvidenceError, "v6 contract"):
                    evidence.validate_live_runtime(
                        fixture.runtime,
                        fixture.profile,
                        expected_runtime=fixture.runtime,
    ...
    def test_reload_command_pack_and_stability_contract_are_exact(self) -> None:
        cases = (
            ...
            (
                "pack resources",
                "pack_resources",
                list(reversed(contract_v6.RELOAD_PACK_RESOURCES)),
            ),
    ...
    def test_all_72_assertions_must_pass_in_exact_order_and_value(self) -> None:
        mutations = (
...
class ArchiveEvidenceTests(unittest.TestCase):
    ...
    def test_resealed_v6_reload_report_tampering_is_rejected(self) -> None:
        cases = (
            (
                "schema downgrade",
                lambda report: report.__setitem__("schema", 2),
            ...
            (
                ...
                lambda report: report["reload"][
                    "enabled_data_pack_names"
                ].remove(contract_v6.RELOAD_PACK_ENABLED_NAME),
            ),
            (
    ...
    def test_resealed_v6_reload_log_tampering_is_rejected(self) -> None:
        cases = (
            (
    ...
    def test_archive_requires_exact_destination_and_ether_source_reload_v6_name(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            fixture = build_fixture(Path(temporary_directory))
            ...
            with self.assertRaisesRegex(
                evidence.EvidenceError,
                "repository path|Ether-source reload profile v6",
            ):
                evidence.validate_archived_evidence(
                    wrong_name,
